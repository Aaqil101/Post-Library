...
def bloom_node_group(context, operator, group_name):
    ...
    bloom.description = ""
    bloom.default_group_node_width = 173
    ...
    bloom_size_socket.default_value = 8.0
    ...
    original_bloom_high.color = (0.4666634798049927, 0.32548609375953674, 0.37254956364631653)
    ...
    original_bloom_high.threshold = 0.4999999403953552
    ...
    blur.color = (0.4666634798049927, 0.32548609375953674, 0.37254956364631653)
    ...
    knee_bloom_high.color = (0.4666634798049927, 0.32548609375953674, 0.37254956364631653)
    ...
    knee_bloom_low.color = (0.4666634798049927, 0.32548609375953674, 0.37254956364631653)
    ...
    knee_bloom_low.size = 8
    knee_bloom_low.streaks = 4
    knee_bloom_low.threshold = 0.0
    knee_bloom_low.use_rotate_45 = True

    #node Switch
    switch = bloom.nodes.new("CompositorNodeSwitch")
    switch.name = "Switch"
    switch.check = False

    #node Switch.001
    switch_001 = bloom.nodes.new("CompositorNodeSwitch")
    switch_001.name = "Switch.001"
    switch_001.check = False
    ...
    original_bloom_low.color = (0.4666634798049927, 0.32548609375953674, 0.37254956364631653)
    ...
    original_bloom_low.threshold = 0.4999999403953552
    ...
    group_input_04.color = (0.23529097437858582, 0.2235269844532013, 0.21568608283996582)
    ...
    #node Reroute_00
    reroute = bloom.nodes.new("NodeReroute")
    reroute.name = "Reroute_00"
    ...
    switch.parent = bloom_high____low
    switch_001.parent = bloom_high____low
    ...
    switch.location = (273.0, -328.0)
    switch_001.location = (273.0, 112.0)
    original_bloom_low.location = (53.0, 172.0)
    group_input_04.location = (-320.0, -260.0)
    reroute.location = (-180.0, -340.0)
    ...
    #switch_001.Image -> blur_mix.Image
    bloom.links.new(switch_001.outputs[0], blur_mix.inputs[2])
    ...
    #switch_001.Image -> blur.Image
    bloom.links.new(switch_001.outputs[0], blur.inputs[0])
    ...
    #knee_bloom_high.Image -> switch.On
    bloom.links.new(knee_bloom_high.outputs[0], switch.inputs[1])
    #knee_bloom_low.Image -> switch.Off
    bloom.links.new(knee_bloom_low.outputs[0], switch.inputs[0])
    #original_bloom_high.Image -> switch_001.On
    bloom.links.new(original_bloom_high.outputs[0], switch_001.inputs[1])
    #original_bloom_low.Image -> switch_001.Off
    bloom.links.new(original_bloom_low.outputs[0], switch_001.inputs[0])
    #group_input_04.Blur Mix -> blur.Size
    bloom.links.new(group_input_04.outputs[6], blur.inputs[1])
    #switch.Image -> reroute.Input
    bloom.links.new(switch.outputs[0], reroute.inputs[0])
    #group_input_00.Image -> original_bloom_low.Image
    bloom.links.new(group_input_00.outputs[0], original_bloom_low.inputs[0])
    #reroute.Output -> reroute_01.Input
    bloom.links.new(reroute.outputs[0], reroute_01.inputs[0])
