...
def passmixer_node_group(context, operator, group_name) -> NodeTree:
    # enable use nodes
    ...
    passmixer.description = (
        "A node group for mixing up the gloss, diff, trans and volume passes"
    )

    # passmixer interface

    # Socket Image
    image_socket = passmixer.interface.new_socket(
    ...
    # Socket Direct
    direct_socket = passmixer.interface.new_socket(
        name="Direct", in_out="INPUT", socket_type="NodeSocketColor"
    )
    ...
    # Socket Indirect
    indirect_socket = passmixer.interface.new_socket(
        name="Indirect", in_out="INPUT", socket_type="NodeSocketColor"
    )
    ...
    # Socket Color
    color_socket = passmixer.interface.new_socket(
    ...
    pmr_group_output.color = Color.DARK_GRAY
    ...
    pmr_group_input.color = Color.DARK_GRAY
    ...
    add_passmixer.color = Color.BROWN
    ...
    multiply_passmixer.color = Color.BROWN
