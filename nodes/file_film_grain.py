...
def file_film_grain_node_group(context, operator, group_name, image_path):
	...
	#Socket image
    image_socket = file_film_grain.interface.new_socket(name = "image", in_out='OUTPUT', socket_type = 'NodeSocketColor')
    image_socket.default_value = (1.0, 1.0, 1.0, 1.0)
    image_socket.attribute_domain = 'POINT'

    #Socket Image
    image_socket = file_film_grain.interface.new_socket(name = "Image", in_out='INPUT', socket_type = 'NodeSocketColor')
    image_socket.default_value = (1.0, 1.0, 1.0, 1.0)
    image_socket.attribute_domain = 'POINT'

    #Socket Fac
    fac_socket = file_film_grain.interface.new_socket(name = "Fac", in_out='INPUT', socket_type = 'NodeSocketFloat')
    ...
    fg_group_output.color = COLORS_DICT["DARK_GRAY"]
    ...
    fg_group_input.color = COLORS_DICT["DARK_GRAY"]
    ...
    image_file_film_grain.color = COLORS_DICT["LIGHT_RED"]
    ...
    overlay_file_film_grain.color = COLORS_DICT["BROWN"]
    ...
    try:
        file_film_grain.links.new(image_file_film_grain.outputs[0], overlay_file_film_grain.inputs[2])
    except Exception as e:
        print("Error: unable to create link between image_file_film_grain and overlay_file_film_grain")
        print(e)

    #fg_group_input.Image -> overlay_file_film_grain.Image
    try:
        file_film_grain.links.new(fg_group_input.outputs[0], overlay_file_film_grain.inputs[1])
    except Exception as e:
        print("Error: unable to create link between fg_group_input and overlay_file_film_grain")
        print(e)

    #overlay_file_film_grain.Image -> fg_group_output.image
    try:
        file_film_grain.links.new(overlay_file_film_grain.outputs[0], fg_group_output.inputs[0])
    except Exception as e:
        print("Error: unable to create link between overlay_file_film_grain and group_output")
        print(e)

    #fg_group_input.Fac -> overlay_file_film_grain.Fac
    try:
        file_film_grain.links.new(fg_group_input.outputs[1], overlay_file_film_grain.inputs[0])
    except Exception as e:
        print("Error: unable to create link between fg_group_input and overlay_file_film_grain")
        print(e)
